...
*/
  ...
  function shortestPath(friends, start, target) {
    ...
    while(queue.length > 0) {
      ...
      if(currentNode === target) return `The path is ${track[start]} by ${distance[currentNode]} distance!`;

      for(let neighbor of friends[currentNode]) {
        if(!explored.has(neighbor)) {
          queue.push(neighbor);
          explored.add(neighbor);
          distance[neighbor] = distance[currentNode] + 1;
          track[neighbor] = currentNode;
        }
      }
    }
