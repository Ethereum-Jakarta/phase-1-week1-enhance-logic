...
function dijkstra(graph, start, end) {
  ...
  for (let town in graph) {
    if (town === start) {
      distances[town] = 0;
      pq.enqueue(town, 0);
    } else {
      distances[town] = Infinity;
      pq.enqueue(town, Infinity);
    }
    previous[town] = null;
  }

  while (!pq.isEmpty()) {
    const currentTown = pq.dequeue();

    for (let neighbor in graph[currentTown]) {
      const distance = distances[currentTown] + graph[currentTown][neighbor];
