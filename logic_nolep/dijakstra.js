...
function dijkstra(graph, start, end) {
   ...
   while(!pq.isEmpty()) {
    ...
    let  neighbor = graph[currentNode];
    for(let neighborName in neighbor) {
        if(explored.has(neighborName)) continue;

        let newDistance = distances[currentNode] + neighbor[neighborName];

        if(newDistance < distances[neighborName]) {
            distances[neighborName] = newDistance;
            previous[neighborName] = currentNode;
            pq.enqueue(neighborName, newDistance);
        }
    }
   }
