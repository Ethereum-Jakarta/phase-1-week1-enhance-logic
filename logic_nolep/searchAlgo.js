...
const searchRotatedArray = (nums, target) => {
    ...
    /* ==== ALGORITHMS =====
    1. Create a state variable called left and right
    2. Cretae while loop and use "left <= right" as a base case
    3. Find a midlle index of array
    4. if element of mid index is a target, retrun a mid index
    5. Check which one that the element is origenized betwen left and right
    6. Check there's a target or not inside that row
    7. retrun -1 if there's no target inside array
    */
    ...
    while(left <= right) {
        ...
        if(nums[left] <= nums[midIndex]) {
            if(target >= nums[left] && target < nums[midIndex]) right = midIndex - 1;
            else left = midIndex + 1;
        } else {
            if(target > nums[midIndex] && target <= nums[right]) left = midIndex + 1;
